...
硬链接没有创建新文件，只是多了一个别名。只是在目录中多了一个记录，同一个I-node号。硬链接本质上是增加了i-node引用计数。删除文件的时候实际上删的也是硬链接计数，计数为0则i-inde标记的存储区域被释放掉，才是真正的删除文件。
...
| 符号     | 作用                 | 说明                                                         |
| -------- | -------------------- | ------------------------------------------------------------ |
| >    >>  | 输出重定向           | 重定向到指定文件，文件不存在则会创建，如果
文件存在， > 会导致之前的内容丢失， >> 则会追加到文件末尾 |
...
注意，一般以如果输出没有重定向，一般以则仍然会在shell 中显示，一般以在后台运行的任务一般把输出进行重定向。
...
而是通过设置 passwd 命令所属用户为 root ，并设置并设置set-user-ID 标志位。这样其他用户在运行 passwd 时就好像是 root 在执行。
